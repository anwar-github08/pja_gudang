...
<body>
      ...
      <div class="col-md-3">
        <div class="card text-white bg-danger">
          ...
          <div class="card-body">
            ...
            <p class="text-white" id="err_tgl"></p>

            <label>Sales</label>
            <select name="id_sales" size="1" id="sales" class="form-control mb-2" style="width: 100%" required>
              <?php while ($lihats = mysqli_fetch_assoc($querys)) :; ?>
                <option value="<?= $lihats['id_sales'] ?>"><?= $lihats['nama_sales'] ?></option>
              <?php endwhile ?>
            </select>
            <p class="text-white" id="err_sales"></p>

            <label>Pelanggan</label>
            <select name="id_pelanggan" size="1" id="pelanggan" class="form-control mb-2" style="width: 100%" required>
              <?php while ($lihatp = mysqli_fetch_assoc($queryp)) :; ?>
                <option value="<?= $lihatp['id_pelanggan'] ?>"><?= $lihatp['nama_pelanggan'] ?></option>
              <?php endwhile ?>
            </select>
            <p class="text-white" id="err_pelanggan"></p>
  ...
  <!-- js barang keluar -->
  <script src="js/js_barang_keluar.js"></script>
